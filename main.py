...
class MultiprocessingScreenCaptureApp:
    def __init__(self, root, fps=16):
        self.root = root
        self.root.title("CCTV Alert System")
        self.root.minsize(968, 432)
        self.fps = fps
    ...
    def refresh_selections(self):
        ...
        self.screen_selector['values'] = [f"{s.name}: {s.width}x{s.height}" for s in screens]
        
        # Get available windows
        windows = gw.getAllTitles()
        self.window_selector['values'] = [w for w in windows if w]
    def start_capture(self):
        ...
        screens = screeninfo.get_monitors()
        screen_mapping = {f"{s.name}: {s.width}x{s.height}": index + 1 for index, s in enumerate(screens)}
        selected_screen = self.screen_var.get()
        display_number = screen_mapping.get(selected_screen, None)
        
        selected_window = self.window_var.get()

        yolo_v8_size = "NANO"
        max_frames_per_second = self.fps
        ...
        collecting_person_patches = "SAVE_OFFICERS_ONLY_EVERY_N_SECONDS/6"
        verbose = False

        # Start new capture process
        self.capture_process = Process(target=capture_and_process, args=(
            self.queue,
            display_number, 
            selected_window,
            yolo_v8_size,
            max_frames_per_second,
            input_frame_dimension,
            object_confidence_threshold,
            remove_oversize_objects,
            person_detection_only,
            segmentation_on_person_option,
            segmentation_on_officers_option,
            collecting_person_patches,
            verbose))
        self.capture_process.start()
    def on_screen_selected(self, event):
        # Clear window selection if a screen is selected
        if self.screen_var.get():
            self.window_selector.set('')
    def on_window_selected(self, event):
        # Clear screen selection if a window is selected
        if self.window_var.get():
            self.screen_selector.set('')
    def update_screen_capture(self):
        if not self.queue.empty():
            screen_np = self.queue.get()

            if isinstance(screen_np, np.ndarray) and screen_np.shape[2] == 3:
                screen_np = cv2.cvtColor(screen_np, cv2.COLOR_BGR2RGB)
                pil_image = Image.fromarray(screen_np)
                canvas_width, canvas_height = int(self.canvas.winfo_width()), int(self.canvas.winfo_height())
                resized_img = pil_image.resize((canvas_width, canvas_height), Image.Resampling.LANCZOS)
                screen_photo = ImageTk.PhotoImage(resized_img)
                self.canvas.create_image(0, 0, anchor=tk.NW, image=screen_photo)
                self.canvas.image = screen_photo

        self.root.after(self.capture_interval, self.update_screen_capture)
    def on_close(self):
        if self.capture_process:
            self.capture_process.terminate()
            self.capture_process.join()
